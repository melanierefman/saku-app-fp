...
public class MarketingCustomerService {
    ...
    public PageResponse<MarketingCustomerItemResponse> findAllPaginated(int page, int size, String search, String tier, UUID karyawanId) {
        Cabang marketingCabang = null;
        if (karyawanId != null) {
            Optional<Karyawan> kOpt = karyawanRepository.findById(karyawanId);
            if (kOpt.isPresent() && kOpt.get().getCabang() != null) {
                marketingCabang = kOpt.get().getCabang();
            }
        }

        final Cabang filterCabang = marketingCabang;
        List<Customer> allCustomers = customerRepository.findAllByOrderByCreatedDateDesc();

        List<MarketingCustomerItemResponse> verifiedCustomers = allCustomers.stream()
    ...
    private boolean isCustomerInBranch(Customer customer, Cabang branch) {
        ...
        // 2. Check customer Domisili (or KTP) province against branch
    ...
    public MarketingCustomerDetailResponse getDetail(UUID customerId) {
        ...
        return MarketingCustomerDetailResponse.builder()
    ...
    private MarketingCustomerItemResponse mapToItemResponse(Customer customer) {
