...
export class PengajuanPinjamanListComponent implements OnInit, OnDestroy {
  ...
  private applySorting(): void {
    ...
    if (field && this.items().length > 0) {
      const sorted = sortTableData<MarketingPengajuanItemResponse>(
        this.items(),
        field,
        dir,
        {
          ...
          tanggalReviewTerakhir: (item) => item.tanggalReviewTerakhir || (item as any).tanggalReviewMarketing || (item as any).tanggalReview,
          status: (item) => this.getStatusLabel(item.status),
        }
      );
