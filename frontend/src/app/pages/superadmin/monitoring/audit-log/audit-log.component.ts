...
export class AuditLogComponent implements OnInit {
  ...
  selectedTanggalAktivitas = signal<Date | null>(null);
  ...
  filteredLogs = computed(() => {
    ...
    const tglYMD = this.formatDateToYMD(this.selectedTanggalAktivitas());
    ...
    let result = list.filter((log) => {
      ...
      // 4. Tanggal Aktivitas Filter
      if (tglYMD) {
        const logDateStr = log.createdDate || log.timestamp || '';
        if (!logDateStr.startsWith(tglYMD)) {
          const parsed = new Date(logDateStr);
  ...
  onTanggalAktivitasChange(date: any): void {
    this.selectedTanggalAktivitas.set(
  ...
  clearFilters(): void {
    ...
    this.selectedTanggalAktivitas.set(null);
    this.currentPage.set(1);
  }
