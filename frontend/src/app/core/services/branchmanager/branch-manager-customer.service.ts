...
} from '../../models/branchmanager/branch-manager-customer.model';

@Injectable({
  providedIn: 'root',
})
export class BranchManagerCustomerService {
  private http = inject(HttpClient);
  private readonly baseUrl = `${environment.apiUrl}/branch-manager/customers`;
  ...
  findAllPaginated(params?: {
    ...
    search?: string;
    tier?: string;
  }): Observable<PageResponse<BranchManagerCustomerItemResponse>> {
    ...
    return this.http
      .get<ApiResponse<PageResponse<BranchManagerCustomerItemResponse>> | any>(this.baseUrl, {
        params: httpParams,
      })
      .pipe(
        map((res: any) => {
        ...
        catchError((err) => {
          console.warn('Failed to fetch branch manager verified customers:', err);
          return of({
            content: [],
            totalElements: 0,
            totalPages: 0,
  ...
  getDetail(customerId: string): Observable<BranchManagerCustomerDetailResponse | null> {
    return this.http
      .get<ApiResponse<BranchManagerCustomerDetailResponse>>(`${this.baseUrl}/${customerId}`)
      .pipe(
